...
//This function will read the channels of the TSL and returns the calculated value to the caller
{
    ...
    x = I2C0_Read (TSL2591_ADDR, ( TSL2591_C0DATAH));
    x <<= 16;
    x |= I2C0_Read (TSL2591_ADDR, ( TSL2591_C0DATAL));
...
while(1)
{
    ...
    {
        lux = GetLuminosity (); //receive lux values
        luxAvg += lux; //sum
    }
    luxAvg = luxAvg/20; //average value of LUX sensor
        ...
        usprintf (HTTP_POST, "GET /update?key=R73A74QVEV8C1XZM&field2=%d&headers=falseHTTP/1.1\nHostapi.thingspeak.com\nConnection:close\Accept*\*\r\n\r\n", luxAvg);
